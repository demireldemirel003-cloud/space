...
export class Game extends Phaser.Scene {
    ...
    update() {
        ...
        // Safely update player if it exists and is active
    ...
    initSounds() {
        // Initialize sound generator for procedural 8-bit sounds
        this.soundGenerator = new SoundGenerator();
    }

    initGameUi() {
        // Create tutorial text
        this.tutorialText = this.add.text(this.centreX, this.centreY, 'Press SPACE to shoot!\nA/D keys to move left/right\nPress 1,2,3 for fire modes', {
        ...
        this.enemiesText = this.add.text(20, 100, 'Enemies: 0', {
        ...
        this.healthText = this.add.text(this.scale.width - 20, 20, 'Health: ♥♥♥', {
        ...
        this.fireModeText = this.add.text(this.scale.width - 20, 60, 'Mode: Single', {
    ...
    initInput() {
        ...
        // Add A/D keys for movement and number keys for fire modes
        this.keys = this.input.keyboard.addKeys('A,D,ONE,TWO,THREE');
        ...
        this.keys.ONE.on('down', () => {
            if (this.gameStarted) {
                this.player.fireMode = 'single';
                this.updateFireModeDisplay();
            }
        });
        
        this.keys.TWO.on('down', () => {
            if (this.gameStarted) {
                this.player.fireMode = 'double';
                this.updateFireModeDisplay();
            }
        });
    ...
    hitPlayer(player, obstacle) {
        // Check if player still exists and is not invulnerable
        ...
        // Clean up the obstacle
        if (obstacle && obstacle.active) {
            obstacle.die();
        }
        
        // The player.hit() method will handle calling die() and GameOver if needed
        // No need to check health here as the Player class manages its own death
    }

    hitEnemy(bullet, enemy) {
        this.updateScore(10);
        this.enemiesKilled++;
        this.enemiesText.setText(`Enemies: ${this.enemiesKilled}`);
        
        // Level up every 20 enemies
        if (this.enemiesKilled % 20 === 0) {
            this.level++;
            this.levelText.setText(`Level: ${this.level}`);
            this.scrollSpeed = Math.min(this.scrollSpeed + 0.5, 5); // Increase scroll speed
        }
        
        bullet.remove();
        
        // Check if enemy will die from this hit
        const willDie = enemy.health <= bullet.getPower();
        
        if (!willDie) {
            // Play hit sound if enemy survives
            this.soundGenerator.generateEnemyHitSound();
        }
    ...
    updateHealthDisplay() {
        ...
        this.healthText.setText(`Health: ${hearts}${emptyHearts}`);
    }

    updateFireModeDisplay() {
        const mode = this.player.fireMode.charAt(0).toUpperCase() + this.player.fireMode.slice(1);
        this.fireModeText.setText(`Mode: ${mode}`);
    }
    ...
    GameOver() {
        ...
        // Disable physics and collision detection to prevent further issues
        if (this.player && this.player.active) {
            this.player.body.enable = false;
        }
        
        // Disable all enemy collision
        ...
        // Transition to GameOver scene after 2 seconds
